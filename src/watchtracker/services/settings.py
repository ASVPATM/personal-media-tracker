...
def persist_env_values(path: Path, changes: dict[str, str | None]) -> None:
    """Atomically update several settings so server activation cannot be half-written."""
    ...
    try:
        ...
        for line in existing:
            ...
            if separator and key in pending:
                ...
                continue
            output.append(line)
        output.extend(f"{key}={value}" for key, value in pending.items() if value is not None)
        path.parent.mkdir(parents=True, exist_ok=True)
        descriptor, temporary_name = tempfile.mkstemp(
            prefix=".watchtracker-env-", dir=path.parent
        )
